...
const standardizePhoneNumbers = (phoneNumbers) => {
  return phoneNumbers.map(number => number.substring(1,4) + number.substring(6,9) + number.substring(10,number.length));
};
...
const unenrollBrook = (roster) => {
  return roster.map(person => person.filter(name => !name.includes('Brook')));
};
